...
| Processing timeout | `MAX_ANALYSIS_SECONDS` (60s) | The gate + detection share **one** wall-clock deadline computed per request (validate runs, then detect runs against the remaining time), so their *combined* time is bounded by the single budget; exceeding it returns **504** to the client. Note: the 504 is a **client-side** deadline — `asyncio.wait_for` stops awaiting the worker, but AnyIO shields the thread, so the in-flight worker finishes its **already-bounded** work in the background rather than being hard-killed. That is acceptable because the CPU-bound pose path is itself frame-bounded (`max_frames`), so the orphaned work is finite; a heavyweight process-pool/hard-kill is intentionally avoided for this hobby v1. |
